...
class SessionItemList extends React.Component {
  ...
  render() {
    return (
      <div>
          <Link className="btn btn-primary my-2" to={`/sessions/main/view/${this.props.idAdress}`}>Return to Session</Link>
          <Table variant="dark">
            <thead>
              <tr>
                <th>#</th>
            ...
            {this.props.sessionCards.length != 0 ? this.renderTable() : "No data to show"}
          </Table>
      </div>
    );
  }
...
export default connect(mapStateToProps, {findCards:sessionCardActions.findAllSessionCards})(SessionItemList);
